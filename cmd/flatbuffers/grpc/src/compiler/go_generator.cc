...
static void GenerateImports(const grpc_generator::Service *service,
  ...
  printer->Print(vars, "\"os\"\n");


  if (bStream){
    printer->Print("\"reflect\"\n");
    printer->Print("\"unsafe\"\n\n");
    printer->Print(vars, "\"$utilsLib$/local_grpc_stream\"\n\n");
  }else{
    printer->Print("\n");
  }
...
static void GenerateClientMethodSignature(const grpc_generator::Method *method,
  ...
  vars["Method"] = exportName(method->name());
  vars["Request"] =
      ", in *" +  method->get_input_type_name() + "T";
...
static void GenerateClientLocalStreamMethod(const grpc_generator::Method *method,
      grpc_generator::Printer *printer, std::map<grpc::string, grpc::string> vars) {
  vars["StreamResponse"] = method->get_input_type_name() + "T";
  vars["StreamRequest"] = method->get_output_type_name() + "T";
  if (vars["LocalStreamType"] == "Client") {
    vars["StreamResponse"] = method->get_output_type_name() + "T";
    vars["StreamRequest"] = method->get_input_type_name() + "T";
  }

  vars["LocalStreamMethod"] = "local" + exportName(method->name()) + vars["LocalStreamType"];

  printer->Print(vars, "type $LocalStreamMethod$Stream struct {\n");
  printer->Print(vars, "\t*local_grpc_stream.Common$LocalStreamType$Stream\n");
  printer->Print(vars, "\tcommonStream *local_grpc_stream.Local$LocalStreamType$Stream\n");
  printer->Print(vars, "}\n");
  ...
  if (genSend) {
    printer->Print(vars, "func (s *$LocalStreamMethod$Stream) Send(data *$StreamRequest$) error {\n");
    printer->Print(vars, "\treturn s.commonStream.Send(data)\n");
    printer->Print(vars, "}\n");
  }
  if (genRecv) {
    printer->Print(vars, "func (s *$LocalStreamMethod$Stream) Recv() (*$StreamResponse$, error) {\n");
    printer->Indent();
    printer->Print(vars, "data, err := s.commonStream.Recv()\n");
    printer->Print(vars, "if err != nil {\n\treturn nil, err\n}\n");
    printer->Print(vars, "data1 := (*$StreamResponse$)(unsafe.Pointer(reflect.ValueOf(data).Pointer()))\n");
    printer->Print(vars, "return data1, nil\n");
    printer->Outdent();
    printer->Print(vars, "}\n");
  }
  if (genSend) {
    printer->Print(vars, "func (s *$LocalStreamMethod$Stream) SendMsg(m any) error {\n");
    printer->Print(vars, "\treturn s.Send(m.(*$StreamRequest$))\n");
    printer->Print(vars, "}\n");
  }
  if (genRecv) {
    printer->Print(vars, "func (s *$LocalStreamMethod$Stream) RecvMsg(m any) error {\n");
    printer->Indent();
    printer->Print(vars, "data, _ := s.Recv()\n");
    printer->Print(vars, "m1 := (*$StreamResponse$)(unsafe.Pointer(reflect.ValueOf(m).Pointer()))\n");
    printer->Print(vars, "*m1 = *data\n");
...
static void GenerateClientMethod(const grpc_generator::Method *method,
  ...
  printer->Indent();
  printer->Print(vars, "go func(){\n");
  printer->Print(vars, "\tlocal$Service$Srv.$Method$(local$Method$ServerStreamer)\n");
  printer->Print(vars, "}()\n");
  printer->Print(vars, "x := &$StreamType${local$Method$ClientStreamer}\n");
  printer->Print(vars, "return x, nil\n");
  printer->Outdent();
  printer->Print(vars, "}\n");
...
void GenerateService(const grpc_generator::Service *service,
  ...
  std::vector<grpc::string> vecStreamName;
  ...
  for (int i = 0; i < service->method_count(); i++) {
    ...
    if (method->NoStreaming()) {
      ...
      // 生成本地调用的函数
      vars["LocalStreamType"] = "Client";
      GenerateClientLocalStreamMethod(method.get(),printer, vars);
      vars["LocalStreamType"] = "Server";
      GenerateClientLocalStreamMethod(method.get(),printer, vars);
      vecStreamName.push_back(exportName(method->name()));
    }
  ...
  {
    for(int i = 0; i < vecStreamName.size(); i++)
    {
      vars["LocalStreamMethod"] = "local" + vecStreamName[i];
      printer->Print(vars, "var $LocalStreamMethod$ClientStreamer *$LocalStreamMethod$ClientStream\n");
      printer->Print(vars, "var $LocalStreamMethod$ServerStreamer *$LocalStreamMethod$ServerStream\n");
    }
    printer->Print(vars, "func init() {\n");
    printer->Indent();
    for(int i = 0; i < vecStreamName.size(); i++)
    {
      printer->Print(vars, "{\n");
      printer->Indent();
      vars["LocalStreamMethod"] = "local" + vecStreamName[i];
      printer->Print(vars, "cs, ss := local_grpc_stream.NewStream()\n");
      printer->Print(vars, "$LocalStreamMethod$ClientStreamer = &$LocalStreamMethod$ClientStream{\n");
      printer->Print(vars, "\tCommonClientStream: cs.CommonClientStream,\n");
      printer->Print(vars, "\tcommonStream:       cs,\n");
      printer->Print(vars, "}\n");

      printer->Print(vars, "$LocalStreamMethod$ServerStreamer = &$LocalStreamMethod$ServerStream{\n");
      printer->Print(vars, "\CommonServerStream: ss.CommonServerStream,\n");
      printer->Print(vars, "\tcommonStream:       ss,\n");
      printer->Print(vars, "}\n");
      printer->Outdent();
      printer->Print(vars, "}\n");
    }
  ...
  printer->Print(vars,"var local$Service$Srv $Service$Server\n");

  printer->Print(vars,
                 "func Register$Service$Server(s $grpc$.ServiceRegistrar, srv "
                 "$Service$Server) {\n");
  printer->Indent();
  printer->Print(vars,"local$Service$Srv = srv\n");
  printer->Print(vars,"if os.Getenv(\"ALL_IN_ONE\") == \"true\" {\n");
...
grpc::string GenerateServiceSource(grpc_generator::File *file,
  ...
  GenerateImports(service, file, printer, vars);
