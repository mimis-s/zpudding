 ...
 private:
  ...
  void GenNativeStruct(const StructDef &struct_def, std::string *code_ptr) {
    ...
    if (!struct_def.fixed) {
      ...
      GenNativeTableMarshalTable(struct_def, code_ptr);
    } else {
  ...
  void GenNativeTableMarshalTable(const StructDef &struct_def, std::string *code_ptr) {
    std::string &code = *code_ptr;
    const std::string struct_type = namer_.Type(struct_def);
 
    code += "func (t *" + NativeName(struct_def) +
            ") MarshalTable(buf []byte) error {\n";
    code += "\tif t == nil {\n\t\treturn errors.New(\"pack is fail "+ NativeName(struct_def) +" is nil\")\n\t}\n";
    
    code += "\tn := flatbuffers.GetUOffsetT(buf)\n";
    code += "\tx := &"+ struct_type +"{}\n";
    code += "\tx.Init(buf, n)\n";
    code += "\tx.UnPackTo(t)\n";
    code += "\treturn nil\n";
